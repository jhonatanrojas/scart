...
function sc_process_css_default($storeId = null) {
        if ($storeId) {
        $cssContent = '';
        if (file_exists($path = resource_path() . '/views/templates/cart-gris/css_default.css')) {
            $cssContent = file_get_contents($path);
        }
        \SCart\Core\Front\Models\ShopStoreCss::insert(['css' => $cssContent, 'store_id' => $storeId, 'template' => 'cart-gris']);
    }
}
...
function sc_template_install_store($storeId = null) {
    ...
    $dataInsert[] = [
        'id'       => sc_uuid(),
        'name'     => 'Category store left (cart-gris)',
        'position' => 'left',
    ...
    $dataInsert[] = [
        'id'       => sc_uuid(),
        'name'     => 'Product last view (cart-gris)',
        'position' => 'left',
        'page'     => '*',
    ...
    $dataInsert[] = [
        'id'       => sc_uuid(),
        'name'     => 'Product special (cart-gris)',
        'position' => 'left',
        'page'     => '*',
    ...
    $idBanner1 = $modelBanner->create(['title' => 'Banner home 1 (cart-gris)', 'image' => '/data/banner/banner-home-1.jpg', 'target' => '_self', 'html' => '', 'status' => 1, 'type' => 'banner']);
    $modelBannerStore->create(['banner_id' => $idBanner1->id, 'store_id' => $storeId]);
    $idBanner2 = $modelBanner->create(['title' => 'Banner home 2 (cart-gris)', 'image' => '/data/banner/banner-home-2.jpg', 'target' => '_self', 'html' => '', 'status' => 1, 'type' => 'banner']);
    $modelBannerStore->create(['banner_id' => $idBanner2->id, 'store_id' => $storeId]);
    $idBanner3 = $modelBanner->create(['title' => 'Banner breadcrumb (cart-gris)', 'image' => '/data/banner/breadcrumb.jpg', 'target' => '_self', 'html' => '', 'status' => 1, 'type' => 'breadcrumb']);
    $modelBannerStore->create(['banner_id' => $idBanner3->id, 'store_id' => $storeId]);
    $idBanner4 = $modelBanner->create(['title' => 'Banner store (cart-gris)', 'image' => '/data/banner/banner-store.jpg', 'target' => '_self', 'html' => '', 'status' => 1, 'type' => 'banner-store']);
...
function sc_template_uninstall_store($storeId = null) {
        if ($storeId) {
        \SCart\Core\Admin\Models\AdminStoreBlockContent::where('template', 'cart-gris')
        ...
        $idBanners = (new \SCart\Core\Front\Models\ShopBanner)
            ...
            ->where($tableBanner.'.title', 'like', '%(cart-gris)%')
        ...
        \SCart\Core\Front\Models\ShopStoreCss::where('template', 'cart-gris')
        ->where('store_id', $storeId)
        ->delete();
    } else {
        ...
        \SCart\Core\Admin\Models\AdminStoreBlockContent::where('template', 'cart-gris')
            ->delete();
        $idBanners = \SCart\Core\Front\Models\ShopBanner::where('title', 'like', '%(cart-gris)%')
            ->pluck('id');
        if ($idBanners) {
            \SCart\Core\Front\Models\ShopBannerStore::whereIn('banner_id', $idBanners)
            ->delete();
            \SCart\Core\Front\Models\ShopBanner::where('title', 'like', '%(cart-gris)%')
            ->delete();
        }
        \SCart\Core\Front\Models\ShopStoreCss::where('template', 'cart-gris')
        ->delete();
