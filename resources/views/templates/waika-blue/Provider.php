...
function sc_process_css_default($storeId = null) {
        if ($storeId) {
        $cssContent = '';
        if (file_exists($path = resource_path() . '/views/templates/waika-blue/css_default.css')) {
            $cssContent = file_get_contents($path);
        }
        \SCart\Core\Front\Models\ShopStoreCss::insert(['css' => $cssContent, 'store_id' => $storeId, 'template' => 'waika-blue']);
    }
}
...
function sc_template_install_store($storeId = null) {
    ...
    $dataInsert[] = [
        'id'       => sc_uuid(),
        'name'     => 'Category store left (waika-blue)',
        'position' => 'left',
    ...
    $dataInsert[] = [
        'id'       => sc_uuid(),
        'name'     => 'Product last view (waika-blue)',
        'position' => 'left',
        'page'     => '*',
    ...
    $dataInsert[] = [
        'id'       => sc_uuid(),
        'name'     => 'Product special (waika-blue)',
        'position' => 'left',
        'page'     => '*',
    ...
    $idBanner1 = $modelBanner->create(['title' => 'Banner home 1 (waika-blue)', 'image' => '/data/banner/banner-home-1.jpg', 'target' => '_self', 'html' => '', 'status' => 1, 'type' => 'banner']);
    $modelBannerStore->create(['banner_id' => $idBanner1->id, 'store_id' => $storeId]);
    $idBanner2 = $modelBanner->create(['title' => 'Banner home 2 (waika-blue)', 'image' => '/data/banner/banner-home-2.jpg', 'target' => '_self', 'html' => '', 'status' => 1, 'type' => 'banner']);
    $modelBannerStore->create(['banner_id' => $idBanner2->id, 'store_id' => $storeId]);
    $idBanner3 = $modelBanner->create(['title' => 'Banner breadcrumb (waika-blue)', 'image' => '/data/banner/breadcrumb.jpg', 'target' => '_self', 'html' => '', 'status' => 1, 'type' => 'breadcrumb']);
    $modelBannerStore->create(['banner_id' => $idBanner3->id, 'store_id' => $storeId]);
    $idBanner4 = $modelBanner->create(['title' => 'Banner store (waika-blue)', 'image' => '/data/banner/banner-store.jpg', 'target' => '_self', 'html' => '', 'status' => 1, 'type' => 'banner-store']);
...
function sc_template_uninstall_store($storeId = null) {
        if ($storeId) {
        \SCart\Core\Admin\Models\AdminStoreBlockContent::where('template', 'waika-blue')
        ...
        $idBanners = (new \SCart\Core\Front\Models\ShopBanner)
            ...
            ->where($tableBanner.'.title', 'like', '%(waika-blue)%')
        ...
        \SCart\Core\Front\Models\ShopStoreCss::where('template', 'waika-blue')
        ->where('store_id', $storeId)
        ->delete();
    } else {
        ...
        \SCart\Core\Admin\Models\AdminStoreBlockContent::where('template', 'waika-blue')
            ->delete();
        $idBanners = \SCart\Core\Front\Models\ShopBanner::where('title', 'like', '%(waika-blue)%')
            ->pluck('id');
        if ($idBanners) {
            \SCart\Core\Front\Models\ShopBannerStore::whereIn('banner_id', $idBanners)
            ->delete();
            \SCart\Core\Front\Models\ShopBanner::where('title', 'like', '%(waika-blue)%')
            ->delete();
        }
        \SCart\Core\Front\Models\ShopStoreCss::where('template', 'waika-blue')
        ->delete();
