            ...
            <tbody class="table-group-divider">
                @if ($cartItem[0]->financiamiento == '1' || $cartItem[0]->financiamiento == 2)
                    @foreach ($cartItem as $item)
                        ...
                        @endphp
                        <tr
                            class="{{ session('arrErrorQty')[$product->id] ?? '' }}{{ session('arrErrorQty')[$product->id] ?? 0 ? ' has-error' : '' }}">
                            <td>{{ $n }}</td>
                            <td colspan="2">
                                <a href="{{ $product->getUrl() }}" class="row_cart-name">
                                    <img class="img_product_card" src="{{ sc_file($product->getImage()) }}"
                                        alt="{{ $product->name }}">
                                </a>
                                <span>
                            ...
                            <td class="cart-col-qty">
                                <div class="cart-qty">
                                    <input style="width: 100px;" type="number" data-id="{{ $item->id }}"
                                        ...
                                        onChange="updateCart($(this));" class="item-qty form-control"
                                        name="qty-{{ $item->id }}" value="{{ $item->qty }}">
                                </div>
