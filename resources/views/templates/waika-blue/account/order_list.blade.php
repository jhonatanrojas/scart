                    ...
                    @foreach ($orders as $order)
                        ...
                        <tr class=" table-striped">
                            ...
                            <td>
                                ...
                                <span
                                    class="badge rounded-pill text-bg-{{ $mapStyleStatus[$order->status] ?? '' }}">{{ $statusOrder[$order->status] }}</span>
                            </td>
                            <td class="text-center">{{ $order->created_at->format('d/m/Y') }}</td>
                            <td>
                                <a href="{{ sc_route('customer.order_detail', ['id' => $order->id]) }}"><i
                                        class="fa fa-indent" aria-hidden="true"></i>
                                    {{ sc_language_render('order.detail') }}</a><br>
                                @if ($order->modalidad_de_compra == 0)
                                    <a href="{{ sc_route('customer.reportar_pago', ['id' => $order->id]) }}"><i
                                            class="fa fa-credit-card" aria-hidden="true"></i> Reportar pago</a>
                                @endif
