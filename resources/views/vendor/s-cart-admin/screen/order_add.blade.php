    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form action="{{ sc_route_admin('admin_order.create') }}" method="post" accept-charset="UTF-8"
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group row  {{ $errors->has('status') ? ' text-red' : '' }}">
                            ...
                            <div class="col-sm-8">
                                <input value="@php echo date("Y-m-d") @endphp" class="form-control fecha"
                                    name="fecha_de_pedido" type="date">
                            </div>
                        </div>
                        ...
                        <div class="form-group row  {{ $errors->has('status') ? ' text-red' : '' }}">
                            <label for="status"
                                class="col-sm-2 col-form-label">{{ sc_language_render('order.status') }}</label>
                            <div class="col-sm-8">
                        ...
                        <div class="form-group row  {{ $errors->has('status') ? ' text-red' : '' }}">
                            ...
                            <div class="col-sm-8">
                                <select class="form-control status" id="modalidad_compra" style="width: 100%;"
                                    name="modalidad_compra">

                                    @foreach ($modalidadVenta as $modalidad)
                                        <option value="{{ $modalidad->id }}" data-tarjeta="{{ $modalidad->es_tarjeta }}"
                                            @if ($modalidad->id == 1) selected @endif>
                        ...
                        <div class="table-responsive tabla-tarjetas">
...
@push('scripts')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('[name="modalidad_compra"]').change(function() {

                let modalidadCompra = $(this).val()
                let es_tarjeta = $("[name='modalidad_compra'] option:selected").data("tarjeta");
                if (es_tarjeta == 1) {
                   var id = $('[name="customer_id"]').val();
                    $.ajax({
                        url: '{{ sc_route_admin('tarjetas.obtenerTarjeta') }}',
                        type: "get",
                        ...
                        success: function(result) {
                            var datosTarjetas = result.datosTarjeta;
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            <input value="${element.id_tarjeta}" data-id="${element.id_tarjeta}" class="checkbox grid-row-checkbox" name="seleccionarTarjeta"  onchange="handleChange(this);"  type="radio" data-id="1"
                                                id="seleccionarTarjeta">
                                        </td>

                                        <td> ${element.nro_tarjeta}</td>
                                        <td>${element.tipo}  </td>

                                        <td>${element.totalTransaccion} </td>
                    ...
                    html +=  ` 
                        ...
                        <tbody>
                            <tr >
                                <td colspan="4">
                                    <p class="text-center"> NO TIENE TARJETA ASOCIADA</p>
                                </td>
