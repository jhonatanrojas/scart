      ...
      {{-- select permission --}}
                            ...
                            <div class="form-group row {{ $errors->has('permission') ? ' text-red' : '' }}">
                                <label for="permission"
                                    class="col-sm-2  control-label">{{ sc_language_render('admin.role.select_permission') }}</label>
                                <div class="col-sm-8">
                                    <select class="form-control input-sm permission select2" multiple="multiple"
                                        data-placeholder="{{ sc_language_render('admin.role.select_permission') }}"
                                        style="width: 100%;" name="permission[]">
                                        <option value=""></option>
                                        @foreach ($permission as $k => $v)
                                            <option value="{{ $k }}"
                                                {{ count($listPermission) && in_array($k, $listPermission) ? 'selected' : '' }}>
                                                {{ $v }}</option>
                                        @endforeach
                                    </select>
                            ...
                            {{-- //select permission --}}
                               ...
                               @php
                               $liststatus = [];
                            $old_status = old('status', $role ? $role->status->pluck('id')->toArray() : '');
                               if (is_array($old_status)) {
                                   foreach ($old_status as $value) {
                                       $liststatus[] = $value;
                                   }
                               }
                           ...
                           <div class="form-group row {{ $errors->has('status') ? ' text-red' : '' }}">
                               <label for="status"
                                   class="col-sm-2  control-label">estatus del pedido</label>
                               <div class="col-sm-8">
                                   <select class="form-control input-sm status select2" multiple="multiple"
                                       data-placeholder="status del pedi"
                                       style="width: 100%;" name="status[]">
                                       <option value=""></option>
                                       @foreach ($statusOrder as $k => $v)
                                           <option value="{{ $k }}"
                                               {{ count($liststatus) && in_array($k, $liststatus) ? 'selected' : '' }}>
                                               {{ $v }}</option>
                                       @endforeach
                                   </select>
                                   @if ($errors->has('permission'))
                           ...
                           {{-- //select status --}}
                            ...
                            <div class="form-group row {{ $errors->has('administrators') ? ' text-red' : '' }}">
                                <label for="administrators"
                                    class="col-sm-2  control-label">{{ sc_language_render('admin.role.select_user') }}</label>
                                <div class="col-sm-8">
                                    <select class="form-control input-sm administrators select2" multiple="multiple"
                                        data-placeholder="{{ sc_language_render('admin.role.select_user') }}"
                                        style="width: 100%;" name="administrators[]">
                                        <option value=""></option>
                                        @foreach ($userList as $k => $v)
                                            <option value="{{ $k }}"
                                                {{ in_array($k, $old_administrators) ? 'selected' : '' }}>
                                                {{ $v }}</option>
                                        @endforeach
                                    </select>
